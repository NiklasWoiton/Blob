...
public class ProtobufUnityCompiler : AssetPostprocessor {
    ...
    public static readonly string prefCSOutputPath = "ProtobufUnity_CSOutputPath";
    public static readonly string prefJSOutputPath = "ProtobufUnity_JSOutputPath";
    ...
    public static string csOutputPath
    {
        get
        {
            return EditorPrefs.GetString(prefCSOutputPath, "");
        }
        set
        {
            EditorPrefs.SetString(prefCSOutputPath, value);
        }
    }

    public static string jsOutputPath
    {
        get
        {
            return EditorPrefs.GetString(prefJSOutputPath, "");
        }
        set
        {
            EditorPrefs.SetString(prefJSOutputPath, value);
        }
    }
    ...
    static void PreferencesItem()
    {
        ...
        EditorGUILayout.HelpBox(@"On Windows put the path to protoc.exe (e.g. \protoc-3.6.0-win32\bin\protoc.exe).", MessageType.Info);
        EditorGUILayout.HelpBox(@"All paths are relative to the Assets folder.", MessageType.Info);
        ...
        EditorGUILayout.HelpBox(@"Leave output folders empty to generate the code into the same folder(s) as each .proto file.", MessageType.Info);
        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.LabelField("Output folder C#", GUILayout.Width(100));
        csOutputPath = EditorGUILayout.TextField(csOutputPath, GUILayout.ExpandWidth(true));
        EditorGUILayout.EndHorizontal();
        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.LabelField("Output folder JS", GUILayout.Width(100));
        jsOutputPath = EditorGUILayout.TextField(jsOutputPath, GUILayout.ExpandWidth(true));
        EditorGUILayout.EndHorizontal();
    ...
    private static bool CompileProtobufSystemPath(string protoFileSystemPath, string[] includePaths)
    {
        ...
        {
            string csOutputPathParam =
                csOutputPath.Length == 0 ?
                Path.GetDirectoryName(protoFileSystemPath) :
                Application.dataPath + csOutputPath;
            string jsOutputPathParam =
                jsOutputPath.Length == 0 ?
                Path.GetDirectoryName(protoFileSystemPath) :
                Application.dataPath + jsOutputPath;

            string options = " --csharp_out \"{0}\"  --js_out=import_style=commonjs,binary:\"{1}\" ";
            foreach (string s in includePaths)
            {
                options += string.Format(" --proto_path \"{0}\" ", s);
            }

            string finalArguments = string.Format("\"{0}\"", protoFileSystemPath) + string.Format(options, csOutputPathParam, jsOutputPathParam);
            ...
            ProcessStartInfo startInfo = new ProcessStartInfo() { FileName = Application.dataPath + excPath, Arguments =  finalArguments};
